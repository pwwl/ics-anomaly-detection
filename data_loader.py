...
def load_train_data(dataset_name, scaler=None, train_shuffle=True, no_transform=False, verbose=False):
    ...
    if dataset_name  == 'BATADAL':
        try:
            df_train = pd.read_csv("data/" + dataset_name + "/train_dataset.csv", parse_dates = ['DATETIME'], dayfirst=True)
        ...
        ks_columns = ['AIT201', 'AIT202', 'AIT203', 'P201', 'P205', 'AIT401', 'AIT402', 'AIT501', 'AIT502', 'AIT503', 'PIT502']
        sensor_cols = [col for col in df_train.columns if col not in (remove_list + ks_columns)]
        ...
        try:
            ...
            raise SystemExit("Unable to find WADI train dataset. Did you request the dataset and process it?")
        remove_list = ['Row', 'Date', 'Time', 'Attack', '2B_AIT_002_PV', '2_LS_001_AL', '2_LS_002_AL', '2_P_001_STATUS', '2_P_002_STATUS']
        ks_columns = ['1_AIT_001_PV', '1_AIT_004_PV', '1_AIT_005_PV', '2_FIC_101_SP', '2_LT_001_PV', '2A_AIT_001_PV', '2A_AIT_003_PV', 
            '2A_AIT_004_PV', '2B_AIT_001_PV', '2B_AIT_004_PV', '3_AIT_002_PV', '3_AIT_005_PV']
        sensor_cols = [col for col in df_train.columns if col not in (remove_list + ks_columns)]
...
def load_test_data(dataset_name, scaler=None, no_transform=False, verbose=False):
    ...
    if dataset_name  == 'BATADAL':
        ...
        remove_list = ['Timestamp', 'Normal/Attack', 'P202', 'P401', 'P404', 'P502', 'P601', 'P603']
        ks_columns = ['AIT201', 'AIT202', 'AIT203', 'P201', 'P205', 'AIT401', 'AIT402', 'AIT501', 'AIT502', 'AIT503', 'PIT502']
        sensor_cols = [col for col in df_test.columns if col not in (remove_list + ks_columns)]
        target_col = 'Normal/Attack'
