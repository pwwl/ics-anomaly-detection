...
# ### RNN classes
# classes
class GatedRecurrentUnit(ICSDetector):
    ...
    def create_model(self):
        ...
        units = self.params['units'] # number of GRU units 
        history = self.params['history'] # Window length to incorporate into GRU prediction
        ...
        if layers > 1:
            ...
            # When feeding an GRU layer to another GRU layer, we need to return sequences.
            lstmlayer = GRU(units, activation=activation, dropout=0.2, return_sequences=True)(input_layer)

            for _ in range(layers - 2):
                lstmlayer = GRU(units, activation=activation, dropout=0.2, return_sequences=True)(lstmlayer)

            # On the last layer, don't return sequences.
            lstmlayer = GRU(units)(lstmlayer)

        else:
            # If just 1 layer, connect directly to input
            lstmlayer = GRU(units)(input_layer)
    ...
    def train_by_idx(self, Xfull, train_idxs, val_idxs, use_callbacks=False, **train_params):
        """ Train GRU, but do indexing in batches
    ...
    def cached_detect(self, instance_errors, theta, window = 1):
        ...
        if window > 1:
            ...
            detection = np.convolve(detection, np.ones(window), 'full') // window
